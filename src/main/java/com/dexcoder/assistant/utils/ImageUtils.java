...
public final class ImageUtils {
     ...
     */
    public static BufferedImage watermarkText(String logoText, Color fontColor, String fontName,
                                              int fontSize, float alpha, int pointX, int pointY,
                                              Integer degree, BufferedImage bufferedImage) {
        try {
            ...
            LOG.warn("给图片添加水印失败", e);
            throw new RuntimeException("给图片添加水印失败");
        }
    }
    ...
    public static int[] getImageWH(byte[] bytes) {
        ...
        try {
            ...
            LOG.error("读取图片长宽出现异常", e);
            throw new AssistantException("读取图片长宽出现异常");
        } finally {
     ...
     * @param targetFile
     */
    public static void resizeCutWriteImage(File srcFile, int newWidth, int newHeight,
                                           File targetFile) {
        byte[] bytes = readFileToByte(srcFile);
     ...
     * @param targetFile
     */
    public static void resizeCutWriteImage(byte[] bytes, int newWidth, int newHeight,
                                           File targetFile) {
        BufferedImage bufferedImage = resizeCutImage(bytes, newWidth, newHeight, true);
        String suffix = targetFile.getName().substring(targetFile.getName().lastIndexOf(".") + 1);
        writeImage(bufferedImage, suffix, targetFile);
    ...
    public static byte[] cutImage(File imageFile, int x, int y, int cutWidth, int cutHeight) {
        try {
            ...
            LOG.error("读取图片失败", e);
            throw new AssistantException("读取图片失败");
        }
    }
    ...
    private static byte[] bufferedImageToByte(BufferedImage bufferedImage, String imageFormat) {
        ...
        try {
            ...
            LOG.error("将BufferedImage转换成byte数组失败", e);
            throw new AssistantException("将BufferedImage转换成byte数组失败");
        } finally {
    ...
    private static byte[] readFileToByte(File srcFile) {
        try {
            ...
            LOG.error("将图片文件转换成byte数组失败", e);
            throw new AssistantException("将图片文件转换成byte数组失败");
        }
    }
    ...
    private static BufferedImage readByteToBufferedImage(byte[] bytes) {
        ...
        try {
            ...
            LOG.error("将byte数组转换成BufferedImage对象失败", e);
            throw new AssistantException("将byte数组转换成BufferedImage对象失败");
        }
    }
    ...
    private static void writeImage(BufferedImage bufferedImage, String formatName, File targetFile) {
        try {
            ...
            LOG.error("图片写入失败", e);
            throw new AssistantException("图片写入失败");
        }
    }
     ...
     */
    private static BufferedImage resizeImage(byte[] bytes, int newWidth, int newHeight,
                                             boolean isDoHeight) {
        ...
        try {
            ...
            LOG.error("读取图片成BufferedImage失败", e);
            throw new RuntimeException("读取图片成BufferedImage失败");
        }
    ...
    public static String getImageFormat(byte[] image) {
        ...
        try {
            ...
            throw new AssistantException("获取图片格式失败");
        } finally {
            try {
                iis.close();
            } catch (IOException e) {
